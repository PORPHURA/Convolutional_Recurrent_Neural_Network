...
n_log = 200  # training batch number before writing a log


# read the write directories
rd_root =   ##### NEED TO FILL IN DATASET PATH
model_name = 'RH_M_chn=%d_mae=%.1f_ssim=%.1f_lr=%.1e-%.1e_n=%d_trial=fix_0.5_n=2_2' % (n_channels, loss_wt['mae'], loss_wt['ssim'], lr_g, lr_d, n_plane)  # the post-fix for tensorboard and model directory
...
if __name__ == '__main__':
    ...
    with tf.device('/gpu:0'):  # change to cpu:0 if no GPU available
        ...
        # create checkpoint
        backbone_model_dir = '.\Models\RH_M_chn=16_mae=3.0_ssim=1.0_lr=1.0e-05-1.0e-06_n=5_trial=mix_backbone'
        ckpt = tf.train.Checkpoint(gen=gen, dis=dis)
        ckpt_mgr = tf.train.CheckpointManager(ckpt, directory=model_dir, max_to_keep=50, checkpoint_name='best_model')
        ckpt_backbone = tf.train.CheckpointManager(ckpt, directory=backbone_model_dir, max_to_keep=2)
        if ckpt_backbone.latest_checkpoint:
            ckpt.restore(ckpt_backbone.latest_checkpoint)
